...
# maintain destination lock." Below the message, the device shows a sequence of changes in frequency
...
numbers = file.map(&:to_i)
numbers.sum # => 510

# --- Part Two --- You notice that the device repeats the same frequency change list over and
...
def first_frequency_twice(numbers)
  ...
  loop do
    numbers.each do |num|
      sum += num
      frequency[sum] += 1
      return sum if frequency[sum] > 1
    end
  end
  0
end
first_frequency_twice(numbers) # => 69074
